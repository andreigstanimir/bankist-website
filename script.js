...
// The this keyword is our opacity, we can't pass an actual argument to a function that is used in addEventListener as a parameter, because that function can only have the event parameter
...
const sectionObserver = new IntersectionObserver(revealSection, {
  root: null,
  threshold: 0.15, // section is revealed when is 15% visible
});
...
const loadImg = function (entries, observer) {
  const [entry] = entries; //only one treshold = only one entry
  if (!entry.isIntersecting) return;
